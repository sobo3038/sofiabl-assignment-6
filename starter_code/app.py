...
def generate_plots(N, mu, sigma2, S):
    ...
    X = np.random.rand(N).reshape(-1, 1)  # Random values for X between 0 and 1
    
    # Generate random Y values with normal additive error (mean mu, variance sigma^2)
    ...
    # Run S simulations to generate datasets and calculate slopes and intercepts
    for _ in range(S):
        X_sim = np.random.rand(N).reshape(-1, 1)  # Generate X values
        Y_sim = mu + np.sqrt(sigma2) * np.random.randn(N)  # Generate Y values
        sim_model = LinearRegression().fit(X_sim, Y_sim)  # Fit model
